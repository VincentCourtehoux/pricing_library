...
def test_up_out_cheaper_than_vanilla():
    np.random.seed(42)
    up_out_price = barrier_option_premium(
        100, 100, 1, 0.05, 0.2, 0, 110, 252, 50000, 'call', 'up-out'
    )
    vanilla_approx = barrier_option_premium(
...
def test_down_out_put_cheaper_than_vanilla():
    np.random.seed(42)
    down_out_price = barrier_option_premium(
...
def test_barrier_far_away_equals_vanilla():
    np.random.seed(42)
    far_barrier_price = barrier_option_premium(
...
def test_barrier_in_vs_out_complement():
    np.random.seed(42)
    up_in_price = barrier_option_premium(
...
if __name__ == "__main__":
    pytest.main(["tests/test_barrier_options_pricing.py"])